...
def gpt(input, t=0, key=None):
    # openai.api_key = key
    openai.api_base = "https://d.lconai.com/v1"
    openai.api_key = "sk-key"
    completion = openai.ChatCompletion.create(
        temperature=0,
        model="gpt-3.5-turbo",  # 选择模型引擎
        messages=[{"role": "user", "content": input}]
     )
    return completion.choices[0].message.content

def parse(rel2id, text,rname):
    ...
    while True:
        ...
        i = temp.find(cons[0])
        print(r)
        parse_item['relation'] = rel2id[rname]
        parse_item['index'] = 0
...
def gen_data(r2desc, rel2id, sample, n=10, t=0, key=None):
    ...
    print(output)
    parse_output = parse(rel2id, output,rname)
